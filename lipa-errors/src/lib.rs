...
#[allow(dead_code)]
pub fn invalid_input<E: ToString, C: Display + Debug + Eq>(e: E) -> LipaError<C> {
    LipaError::InvalidInput { msg: e.to_string() }
}

#[allow(dead_code)]
pub fn runtime_error<E: ToString, C: Display + Debug + Eq>(code: C, e: E) -> LipaError<C> {
...
pub fn permanent_failure<E: ToString, C: Display + Debug + Eq>(e: E) -> LipaError<C> {
    LipaError::PermanentFailure { msg: e.to_string() }
}

pub type LipaResult<T, C> = Result<T, LipaError<C>>;

pub trait LipaResultTrait<T, C> {
    ...
    fn lift_invalid_input(self) -> LipaResult<T, C>;

    fn prefix_error<M: ToString + 'static>(self, msg: M) -> LipaResult<T, C>;
}

impl<T, C> LipaResultTrait<T, C> for LipaResult<T, C> {
    fn lift_invalid_input(self) -> LipaResult<T, C> {
    ...
    fn prefix_error<M: ToString + 'static>(self, prefix: M) -> LipaResult<T, C> {
        self.map_err(|e| match e {
...
pub trait MapToLipaError<T, E: ToString, C> {
    fn map_to_invalid_input<M: ToString>(self, msg: M) -> LipaResult<T, C>;
    fn map_to_runtime_error<M: ToString>(self, code: C, msg: M) -> LipaResult<T, C>;
    fn map_to_permanent_failure<M: ToString>(self, msg: M) -> LipaResult<T, C>;
}

impl<T, E: ToString, C> MapToLipaError<T, E, C> for Result<T, E> {
    fn map_to_invalid_input<M: ToString>(self, msg: M) -> LipaResult<T, C> {
...
pub trait MapToLipaErrorForUnitType<T, C> {
    fn map_to_invalid_input<M: ToString>(self, msg: M) -> LipaResult<T, C>;
    fn map_to_runtime_error<M: ToString>(self, code: C, msg: M) -> LipaResult<T, C>;
    fn map_to_permanent_failure<M: ToString>(self, msg: M) -> LipaResult<T, C>;
}

impl<T, C> MapToLipaErrorForUnitType<T, C> for Result<T, ()> {
    fn map_to_invalid_input<M: ToString>(self, msg: M) -> LipaResult<T, C> {
...
mod tests {
    ...
    fn test_lift_invalid_input() {
        let result: LipaResult<(), TestRuntimeErrorCode> =
        ...
        let result: LipaResult<(), TestRuntimeErrorCode> = Err(runtime_error(
